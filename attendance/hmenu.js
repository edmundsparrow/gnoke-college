 ...
 * Drop in the site root. Each page loads it via <script src="/hmenu.js"></script>
 */

(function () {
  const PAGES = [
    { path: "/ledger/",   label: "Student Ledger",   icon: "📋", sub: "Individual records" },
    { path: "/register/", label: "Weekly Register",   icon: "📅", sub: "Class presence by week" },
    { path: "/analysis/", label: "Term Analysis",     icon: "📊", sub: "Master analytics" },
    { path: "/sessions/", label: "Session Register",  icon: "🕐", sub: "Daily AM/PM sessions" },
  ];

  // Detect current page by pathname
  ...
  drawer.innerHTML = `
    ...
    <nav class="hmenu-nav">
      ${PAGES.map(p => `
        <a class="hmenu-item ${currentPath === p.path ? "active" : ""}" href="${p.path}">
